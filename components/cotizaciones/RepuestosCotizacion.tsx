...
import { Plus, Trash2, Search, AlertTriangle, Loader2, Clock } from "lucide-react";

interface CotizacionRepuesto {
  id: number;
...
export function RepuestosCotizacion({
  cotizacionId,
  editable = true,
  modeloId,
  vehiculo,
  ...
  const [items, setItems] = useState<CotizacionRepuesto[]>([]);
  ...
  const loadItems = useCallback(async () => {
    const res = await fetch(`/api/cotizacion-repuestos?cotizacion_id=${cotizacionId}`);
  ...
  async function handleAgregar() {
    ...
    try {
      const res = await fetch("/api/cotizacion-repuestos", {
        ...
        body: JSON.stringify({ cotizacion_id: cotizacionId, repuesto_id: repuestoSel.id, cantidad }),
      });
  ...
  async function handleEliminar(id: number) {
    await fetch(`/api/cotizacion-repuestos/${id}`, { method: "DELETE" });
    loadItems();
  }
  ...
  return (
    <div className="space-y-4">
            ...
            <div className="flex items-center gap-2 p-3 bg-zinc-50 rounded-lg border border-zinc-200">
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-zinc-500">
                  Stock actual: <span className={repuestoSel.stock_actual <= 0 ? "text-amber-600 font-medium" : "text-green-600 font-medium"}>{repuestoSel.stock_actual} {repuestoSel.unidad}</span>
                  ...
                  {catalogoFiltrado.map(r => (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-3 flex-shrink-0">
                        <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${r.stock_actual <= 0 ? "bg-amber-100 text-amber-700" : "bg-green-100 text-green-700"}`}>
      ...
      {loading ? (
        ...
        <p className="text-sm text-zinc-400 italic">Sin repuestos del inventario en esta cotización.</p>
      ) : (
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-zinc-50 border-b border-zinc-200">
                ...
                <th className="text-center px-3 py-2 text-xs font-semibold text-zinc-500">Estado</th>
