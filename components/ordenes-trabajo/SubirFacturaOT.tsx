  ...
  return (
    <>
      ...
      <Dialog open={open} onOpenChange={val => { if (!val) resetAndClose(); }}>
        <DialogContent
          className={step === 3 ? 'max-w-4xl flex flex-col max-h-[90vh]' : 'max-w-md'}
        ...
        >
          ...
          {step === 1 && (
            <>
              ...
              <DialogFooter>
                <button type="button" onClick={resetAndClose} className="px-4 py-2 text-sm text-zinc-600 border border-zinc-300 rounded-lg hover:bg-zinc-50 transition-colors">
                  Cancelar
                </button>
                <button
                  type="button"
          ...
          {step === 3 && (
            <>
              ...
              <div className="space-y-5 py-2 overflow-y-auto flex-1">
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                  {[
                    ...
                    { label: 'Proveedor', value: proveedorNombre, set: setProveedorNombre, placeholder: 'Nombre del proveedor' },
                    { label: 'RUT proveedor', value: proveedorRut, set: setProveedorRut, placeholder: '12.345.678-9' },
                  ].map(f => (
                    <div key={f.label}>
                      <label className="text-xs text-zinc-500 mb-1 block">{f.label}</label>
                      <input className="w-full border border-zinc-200 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-1 focus:ring-zinc-900" value={f.value} onChange={e => f.set(e.target.value)} placeholder={f.placeholder} />
                    </div>
                  ))}
                  <div>
                ...
                <div>
                        ...
                        {items.map(item => (
                          <tr key={item.localId} className="border-b border-zinc-100 last:border-0">
                            <td className="px-3 py-2">
                              <input className="w-full border border-zinc-200 rounded px-2 py-1 focus:outline-none focus:ring-1 focus:ring-zinc-900" value={item.nombre} onChange={e => updateItem(item.localId, 'nombre', e.target.value)} placeholder="Descripción" />
                            </td>
                            <td className="px-2 py-2">
                              <input type="number" min="1" step="1" className="w-16 border border-zinc-200 rounded px-2 py-1 text-right focus:outline-none focus:ring-1 focus:ring-zinc-900" value={item.cantidad} onChange={e => updateItem(item.localId, 'cantidad', parseFloat(e.target.value) || 1)} />
                            </td>
                            <td className="px-2 py-2">
                              <input type="number" min="0" step="100" className="w-28 border border-zinc-200 rounded px-2 py-1 text-right focus:outline-none focus:ring-1 focus:ring-zinc-900" value={item.precioUnitario} onChange={e => updateItem(item.localId, 'precioUnitario', parseFloat(e.target.value) || 0)} />
                            </td>
                            <td className="px-2 py-2 text-right text-zinc-600 font-medium tabular-nums">
                              {formatCLP(item.cantidad * item.precioUnitario)}
                            </td>
                            <td className="px-2 py-2">
                              <select className="w-full border border-zinc-200 rounded px-2 py-1 focus:outline-none focus:ring-1 focus:ring-zinc-900 bg-white" value={item.matchKey ?? ''} onChange={e => updateItem(item.localId, 'matchKey', e.target.value || null)}>
                            ...
                            <td className="px-1 py-2 text-center">
                              <button type="button" onClick={() => removeItem(item.localId)} className="p-1 rounded hover:bg-red-50 text-zinc-300 hover:text-red-500 transition-colors">
                  ...
                  <button type="button" onClick={addItem} className="mt-2 flex items-center gap-1.5 text-xs text-zinc-500 hover:text-zinc-800 transition-colors">
              ...
              <DialogFooter className="flex-shrink-0 border-t border-zinc-100 pt-4 mt-2">
                ...
                <button type="button" onClick={() => setStep(1)} className="px-4 py-2 text-sm text-zinc-600 border border-zinc-300 rounded-lg hover:bg-zinc-50 transition-colors">
                  Volver
                </button>
                <button
                  type="button"
