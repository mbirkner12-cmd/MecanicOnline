...
import { cotizaciones, vehiculos, clientes, recepciones } from '@/lib/db/schema';
...
export async function POST(
  ...
  try {
    ...
    const pdfBuffer = await renderToBuffer(
      ...
      React.createElement(CotizacionDocument, {
        data: {
          ...
          cliente: row.cliente ? { ...row.cliente, rut: row.cliente.rut ?? "" } : null,
        },
      })
    ) as Buffer;

    // Upload to Vercel Blob (overwrite same file each time)
    const blob = await put(
      `cotizaciones/${row.numero}.pdf`,
      pdfBuffer,
    ...
    console.error('POST /api/cotizaciones/[id]/pdf-share error:', error);
